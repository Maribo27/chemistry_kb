/* --- src/mass_enter-common.js --- */
var MassEnterComponent = {};
...
/* --- src/mass_enter-paintPanel.js --- */
/**
 * Paint panel.
 */
MassEnterComponent.PaintPanel = function (containerId) {
	this.containerId = containerId;
};

MassEnterComponent.PaintPanel.prototype = {
	...
	_findChemicalAmount: function (system_idtf) {
		SCWeb.core.Server.resolveScAddr(["ui_menu_find_mass_of_substance"],
		function (data) {
			var cmd = data["ui_menu_find_mass_of_substance"];
	...
	_createValue: function(elementExample, m, measur) {
		...
		var self = this;
		if (measur === "nrel_chemical_amount") {
			SCWeb.core.Server.resolveScAddr(['value', 'nrel_chemical_amount', 'lang_ru'], function (keynodes) {
				var nrelChemicalAmount = keynodes['nrel_chemical_amount'];
				...
				window.sctpClient.create_node(sc_type_const).done(function (val) {
					...
					window.sctpClient.create_arc(sc_type_arc_common | sc_type_const, elementExample, val).done(function (commonArcTwo) {
						window.sctpClient.create_arc(sc_type_arc_pos_const_perm, nrelChemicalAmount, commonArcTwo);
						window.sctpClient.create_arc(sc_type_arc_pos_const_perm, value, val);
						self._createChemicalAmountMeasurement(val, m, elementExample);
					});
				});
			});
		}
	...
	_createChemicalAmountMeasurement: function (val, m, elementExample) {
	...
	_createChemicalAmountNumber: function (m, meas, elementExample) {
		console.log('creating number');
		var self = this;    	
			...
			window.sctpClient.create_node(sc_type_const).done(function (num) {
				...
				window.sctpClient.create_link().done(function (numName) {
					...
					window.sctpClient.create_arc(sc_type_arc_common | sc_type_const, num, numName).done(function (commonArcFour) {
						window.sctpClient.create_arc(sc_type_arc_pos_const_perm, window.scKeynodes.nrel_system_identifier, commonArcFour);
						window.sctpClient.create_arc(sc_type_arc_pos_const_perm, number, num);
						self._createMolMeasurement(meas, num, elementExample);
					});
				});
				window.sctpClient.create_link().done(function (link_addr) {
					window.sctpClient.set_link_content(link_addr, m);
	...
	_createMolMeasurement: function (meas, numb, elementExample) {
		var self = this;
		console.log('creating mols for numb',numb);
		SCWeb.core.Server.resolveScAddr(['rrel_mol'], function (keynodes) {
			var rrel_mol = keynodes['rrel_mol'];
			window.sctpClient.create_arc(sc_type_arc_pos_const_perm, meas, numb).done(function (arcOne) {
				window.sctpClient.create_arc(sc_type_arc_pos_const_perm, rrel_mol, arcOne).done(function () {
					self._findChemicalAmount(elementExample);
				});
			});
		});
	...
	_createLiterMeasurement: function (meas, numb, elementExample) {
		var self = this;
		console.log('creating liters for numb',numb);
...
/* --- src/mass_enter-component.js --- */
/**
 * MassEnterComponent component.
 */
MassEnterComponent.DrawComponent = {
    ext_lang: 'mass_enter_component',
    formats: ['format_mass_enter_json'],
    struct_support: true,
    factory: function (sandbox) {
        return new MassEnterComponent.DrawWindow(sandbox);
    }
};

MassEnterComponent.DrawWindow = function (sandbox) {
    this.sandbox = sandbox;
    this.paintPanel = new MassEnterComponent.PaintPanel(this.sandbox.container);
...
SCWeb.core.ComponentManager.appendComponentInitialize(MassEnterComponent.DrawComponent);
